...
module.exports = function (app, passport) {
	...
	app.post("/addDoctor", function(req,res){
		...
		var query = "INSERT INTO doctor_1 (doc_name, contact, specialization) VALUES (?,?,?)";
	...
	app.get("/inventory", isLoggedIn, function(req, res){
		...
		connection.query(query, function(err, rows){
			...
			connection.query(query1, function(err, rows1){
				var query3 = "SELECT employee.emp_name, employee.emp_id, login.role \
				...
				connection.query(query3, function(err,rows2){
					...
					connection.query(query4, function(err, rows3){
						res.render('inventory.ejs', {user: req.user, rows: rows, rows1: rows1, rows2: rows2, rows3: rows3});				
					});
				});
			});
	...
	app.post("/addStock", function(req,res){
		...
		var query = "INSERT INTO inventory (med_id, expiry_date, total_number, emp_id) \
		VALUES (?,?,?,?)";
		connection.query(query, [medId, expiryDate, totalStock, associatedEmpId], function(err,rows){
			if (err) {
				console.log(err);
			}
	...
	app.post('/addMedicine', function(req,res){
		...
		var query = "INSERT INTO medicine (med_name, mrp, primary_drug, dosage, company_id) \
		VALUES (?,?,?,?,?)";
		connection.query(query, [medName, medMRP, primaryDrug, dosage, companyId], function(err,rows){
			if (err) {
				console.log(err);
			}
			res.redirect('/inventory');

...
function addMed(medIdStockId){
  ...
  cell1.innerHTML = "<btn class='btn btn-dark btn-sm' onclick='removeMed("+count+");'><i class='fas fa-trash'></i></btn>";
  var cell2 = row.insertCell(1);
  cell2.innerHTML = 1+Number(count);
  var cell3 = row.insertCell(2);
  cell3.innerHTML = "<input id='medName"+count+"' name='medName"+count+"' readonly class='form-control-plaintext' type='text' oninput='addOtherFields('" + count + "');' value='" + medName + "'>";
  var cell4 = row.insertCell(3);
  cell4.innerHTML = "<input id='medId"+count+"' name='medId"+count+"' readonly class='form-control-plaintext' type='number' value='" + medId + "'>";
  var cell5 = row.insertCell(4);
  cell5.innerHTML = "<input id='medStockId"+count+"' name='medStockId"+count+"' readonly class='form-control-plaintext' type='number' value='" + medStockId + "'>";
  ...
  cell9.innerHTML = "<input id='medTotal"+count+"' name='medTotal"+count+"' type='number' class='form-control-plaintext' readonly step='0.1'>";
  ++count;
}
function removeMed(rowNo){
  --count;
  document.getElementById("billTable").getElementsByTagName('tbody')[0].deleteRow(rowNo);
  ...
  for (var i=0;i<tr.length;++i){
    ...
    for (var j=2;j<9;++j){
      ...
      tr[i].cells[j].getElementsByTagName('input')[0].id = inputId.substring(0,inputId.length-1)+i;
      tr[i].cells[j].getElementsByTagName('input')[0].name = inputId.substring(0,inputId.length-1)+i;
    }
    tr[i].cells[2].getElementsByTagName('input')[0].setAttribute("oninput", "addOtherFields("+i+")");
    tr[i].cells[6].getElementsByTagName('input')[0].setAttribute("oninput", "calcTotal("+i+")");

    tr[i].cells[0].getElementsByTagName('btn')[0].setAttribute("onclick", "removeMed("+i+")");
  }
}